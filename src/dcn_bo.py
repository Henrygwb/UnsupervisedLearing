...
#        return ((cost1, cost2, cost3, grad_, param_), updates)
    ...
    def build_finetune_functions(self, datasets, centers, batch_size, mu, learning_rate):
        ...
        cost, updates = self.finetune_cost_updates(centers,mu,learning_rate=learning_rate)
...
def load_data_shared(train_x, train_y, batch_size, MNIST = 1):
    """
...
def test_SdC(train_x, train_y, test_x, test_y, Init='', lbd=.01,
             beta=1, finetune_lr=0.005, mu=0.9, pretraining_epochs=50,
             pretrain_lr_base=0.001, training_epochs=150,
...
#    inputargs, _ = arguments() # get the input arguments
    ...
    datasets = load_data_shared(train_x, train_y, batch_size)
...
#    hidden_array  = normalize(hidden_array, norm='l2', axis=1)
    ...
    acc = numpy.round(metrics.acc(label_true, idx), 5)
    nmi = numpy.round(metrics.nmi(label_true, idx), 5)
    ari = numpy.round(metrics.ari(label_true, idx), 5)
    print '****************************************'
...
#         evaluate the clustering performance every 5 epoches
    ...
    test_set_x, _ = shared_dataset((test_x, test_y))

    n_test_batches = test_set_x.shape[0]
    n_test_batches /= batch_size

    out_sdc_test = theano.function(
        [index],
        outputs=out,
    ...
    for batch_index in xrange(n_test_batches):
        hidden_val.append(out_sdc_test(batch_index))
    hidden_array = numpy.asarray(hidden_val)
    hidden_size = hidden_array.shape
    hidden_array = numpy.reshape(hidden_array,
                                 (hidden_size[0] *
    ...
    acc = numpy.round(metrics.acc(test_y, idx), 5)
    nmi = numpy.round(metrics.nmi(test_y, idx), 5)
    ari = numpy.round(metrics.ari(test_y, idx), 5)
    print '****************************************'
    print('acc = %.5f, nmi = %.5f, ari = %.5f.' % (acc, nmi, ari))
